...
if [[ -z ${POSTGRES_URL} ]]; then
  export POSTGRES_URL=postgresql+psycopg2://postgres:<passowrd>@<server>:5432/$POSTGRES_DB
fi
export RABBIT_FORMALIZATION_TRAINER=formalizationtrainer
export MODEL_PATH=somethingfailure
...
while true; do
  echo "$(date): Starting GPTFormalization worker (PID $$)"
  $PYTHON <path-to-formalizer-gpt>

  exit_code=$?
  if [ $exit_code -ne 0 ]; then
    echo "$(date): formalizer_gpt.py exited with code $exit_code — sending Telegram alert"
    $PYTHON $NOTIFY_SCRIPT --fail \
        --error "formalizer_gpt.py on $(hostname) exited with code $exit_code"
  fi

  echo "$(date): formalizer_gpt.py exited with code $exit_code — restarting in 5 seconds..."
  sleep 5
done
