...
while true; do
  echo "$(date): Starting search worker (PID $$)"
  $PYTHON <path-to-repository>/diophantineequations/distributed/workers/search/search.py

  exit_code=$?
  if [ $exit_code -ne 0 ]; then
    echo "$(date): search.py exited with code $exit_code — sending Telegram alert"
    $PYTHON $NOTIFY_SCRIPT --fail \
        --error "search.py on $(hostname) exited with code $exit_code"
  fi

  echo "$(date): search.py exited with code $exit_code — restarting in 5 seconds..."
  sleep 5
done
