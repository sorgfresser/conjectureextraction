...
export RABBIT_TRAINER_EXCHANGE=trainer
export ACCELERATE_BIN=<path-to-accelerate-bin>
export ACCELERATE_CONFIG=<repository-path>/accelerate.yml
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
export TWILIO_ACCOUNT_SID=<your_twilio_account_sid>
export TWILIO_AUTH_TOKEN=<your_twilio_auth_token>
export TWILIO_FROM_NUMBER=<twilio_phone_number>
export ALERT_PHONE_NUMBER=<phone_number_to_call>
...
while true; do
  echo "$(date): Starting trainer (PID $$)"
  $PYTHON <path-to-repo>/diophantineequations/distributed/workers/trainer.py

  exit_code=$?
  if [ $exit_code -ne 0 ]; then
    echo "$(date): trainer.py exited with code $exit_code — sending Telegram alert"
    $PYTHON $NOTIFY_SCRIPT --fail \
        --error "trainer.py on $(hostname) exited with code $exit_code"
  fi

  echo "$(date): trainer.py exited with code $exit_code — restarting in 10 seconds..."
  sleep 10
done
