...
export MODEL_PATH=kfdong/STP_model_Lean_0320
...
while true; do
  echo "$(date): Starting NoSearch worker (PID $$)"
  $PYTHON <path-to-repository>/diophantineequations/distributed/workers/search/no_search.py

  exit_code=$?
  if [ $exit_code -ne 0 ]; then
    echo "$(date): no_search.py exited with code $exit_code — sending Telegram alert"
    $PYTHON $NOTIFY_SCRIPT --fail \
        --error "no_search.py on $(hostname) exited with code $exit_code"
  fi

  echo "$(date): no_search.py exited with code $exit_code — restarting in 5 seconds..."
  sleep 5
done
